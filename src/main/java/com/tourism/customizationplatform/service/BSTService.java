...
public class BSTService {
    ...
    public TravelPackage findPackageById(String id) {
        return findPackageByIdRec(root, UUID.fromString(id));
    }

    private TravelPackage findPackageByIdRec(Node node, UUID id) {
        if (node == null) {
            return null;
        }
        int compareResult = id.compareTo(node.data.getId());
        if (compareResult == 0) {
            return node.data;
        } else if (compareResult < 0) {
            return findPackageByIdRec(node.left, id);
        } else {
            return findPackageByIdRec(node.right, id);
        }
    }
